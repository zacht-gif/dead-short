...
const MUTATIONS = [
  ...
  },
  {
    ...
    expect: 'no longer reserves height for the Trace row',
    note: 'the Trace row falls off the bottom of a landscape phone',
  },
  {
  ...
  {
    // The single branch that keeps this a puzzle. With the input spending too,
    // the clock and the hand both advance the board and drawing fast beats
    // drawing well - which looks like nothing worse than a quick player.
    id: 'input-spends-a-move-as-well',
    file: 'index.html',
    find: "\n    startMetronome();\n    return true;",
    replace: "\n    startMetronome(); stepTick();\n    return true;",
    expect: 'takeTurn() spends a move',
    note: 'hand speed becomes skill and the game is an action game',
  },
  {
    // Was 'drawing-never-starts-the-clock', aimed at planTo. It could not be
    // killed, because both ways into planTo have already started the clock -
    // an unkillable mutation is a line with no observable effect, and those
    // calls are gone now. This is the same claim aimed where it is load-bearing.
    id: 'keyboard-never-starts-the-clock',
    file: 'index.html',
    find: "        sfx.click();\n      }\n      takeTurn();",
    replace: "        sfx.click();\n      }",
    expect: 'in practice, Tab picking a wire costs a move',
    note: 'a keyboard player picks up wires free in the mode where that is a move',
  },
  {
    id: 'score-stops-being-a-time',
    file: 'index.html',
    find: "  function fmtTicks(t){ return fmtSeconds(t / TRIAL_HZ); }",
    replace: "  function fmtTicks(t){ return t === 1 ? '1 move' : t + ' moves'; }",
    expect: 'three ticks read as one second',
    note: 'every score reverts to moves in a game whose only score is the clock',
  },
  {
    id: 'shorting-costs-nothing',
    file: 'index.html',
    find: "  function currentScore(){ return tickCount + zapCount * ZAP_PENALTY_TICKS; }",
    replace: "  function currentScore(){ return tickCount; }",
    expect: 'shorting is not free on the clock',
    note: 'the fastest line through a hot cell becomes walking straight into it',
  },
  {
    // ---- practice: the clock off, and nothing written down ----
    id: 'practice-on-by-default',
    file: 'index.html',
    find: "                             practice:false};",
    replace: "                             practice:true};",
    expect: 'practice is off until asked for',
    note: 'everybody gets the untimed mode and nobody records a score again',
  },
  {
    id: 'practice-records-a-best',
    file: 'index.html',
    find: "      const improved = practice ? false : setBestIfBetter(level, score);",
    replace: "      const improved = setBestIfBetter(level, score);",
    expect: 'a practice win records no best',
    note: 'the untimed mode becomes the fastest way to farm every medal in the game',
  },
  {
    id: 'practice-unlocks-cosmetics',
    file: 'index.html',
    find: "      if(zapCount === 0 && !practice) markCleanClear(level);",
    replace: "      if(zapCount === 0) markCleanClear(level);",
    expect: 'a practice win unlocks no cosmetic',
    note: 'every skin is earnable with the clock off, which is what earns them',
  },
  {
    id: 'practice-starts-the-metronome',
    file: 'index.html',
    find: "    if(metronomeId !== null || settings.practice) return;",
    replace: "    if(metronomeId !== null) return;",
    expect: 'startMetronome refuses while practice is on',
    note: 'the untimed mode is timed after all, and each action also costs a beat',
  },
  {
    id: 'practice-board-never-moves',
    file: 'index.html',
    find: "    if(settings.practice){ stepTick(); stepAnimUntil = clockNow() + STEP_ANIM_MS; return true; }",
    replace: "    if(settings.practice){ return true; }",
    expect: 'practice no longer advances the board',
    note: 'no clock and no step, so a practice board sits frozen forever',
  },
  {
    id: 'wait-is-live-with-the-clock',
    file: 'index.html',
    find: "    if(!settings.practice) return;",
    replace: "    ;",
    expect: 'waitMove() no longer refuses outside practice',
    note: 'the keyboard reaches waitMove without the button, for a free extra move',
  },
  {
    id: 'wait-button-never-hides',
    file: 'index.html',
    find: "    if(waitBtnEl) waitBtnEl.classList.toggle('hidden', !settings.practice);",
    replace: "    ;",
    expect: 'the Wait button is no longer hidden',
    note: 'a dead control sits under the board in the mode that cannot use it',
  },
  {
    id: 'practice-banner-never-shows',
    file: 'index.html',
    find: "    if(bannerPracticeEl) bannerPracticeEl.classList.toggle('hidden', !settings.practice);",
    replace: "    ;",
    expect: 'practice swaps the rules on screen',
    note: 'practice is played under rules that describe a clock which is not running',
  },
  {
    // The FOURTH copy of the rules, and the only one nobody can see. It went
    // stale for a day after the metronome landed and nothing noticed.
    id: 'screen-reader-rules-stick',
    file: 'index.html',
    find: "      boardKeysEl.textContent = settings.practice",
    replace: "      boardKeysEl.textContent = false",
    expect: 'practice swaps the screen-reader rules',
    note: 'a screen reader describes the timed game to a player in practice',
  },
  {
